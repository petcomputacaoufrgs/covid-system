...
function validaPerfilPaciente(){
     
    var strTipo = document.getElementById("idPerfilPaciente");
    //var tipoAmostra = '<?=$objTipoAmostra->getTipo() ?>';
    
    var div_feedback = document.getElementById("feedback_perfilPaciente");
    ...
    if(strTipo.value.length == 0 || strTipo.value.length > 50 ){ // não digitou nada
        ...
        if(strTipo.value.length > 50 )div_feedback.innerHTML = " Digite um perfil de paciente com menos 50 caracteres. ";
    }
...
function validaCaractere(){
    ...
    var strTipo = document.getElementById("idCaracterePerfilPaciente");
    //var tipoAmostra = '<?=$objTipoAmostra->getTipo() ?>';
    
    var div_feedback = document.getElementById("feedback_caracterePerfilPaciente");
    //console.log(strTipo.value); 
    ...
    if(strTipo.value.length == 0 || strTipo.value.length > 1 ){ // não digitou nada
        ...
        div_feedback.classList.add("invalid-feedback");
        if(strTipo.value.length == 0 )div_feedback.innerHTML = " Digite um perfil de paciente. ";
        if(strTipo.value.length > 1 )div_feedback.innerHTML = " Apenas 1 caractere é permitido. ";
    }
    else if(strTipo.value.length > 0){
         strTipo.classList.add("is-valid");
        if(strTipo.classList.contains("is-invalid")) strTipo.classList.remove("is-invalid");
