...
class InfosTuboRN{
    ...
    public static $TST_LOCAL_ERRADO_POCO =  'K';
    public static $TST_AGUARDANDO_BANCO_RNA =  'A';
    ...
    //observações
    public static $O_PREPARO_LOTE_APAGADO= 'Preparo lote foi apagado';
    ...
    public static function listarValoresTipoEtapa(){
        try {
            ...
            $objSituacao = new Situacao();
            $objSituacao->setStrTipo(self::$TP_DIAGNOSTICO);
    ...
    public static function listarValoresTipoStaTubo(){
        try {
            ...
            $objSituacao = new Situacao();
            $objSituacao->setStrTipo(self::$TST_LOCAL_ERRADO_POCO);
            $objSituacao->setStrDescricao('Problemas RTqPCR');
            $arrObjTStaTubo[] = $objSituacao;

            $objSituacao = new Situacao();
            $objSituacao->setStrTipo(self::$TST_AGUARDANDO_BANCO_RNA);
            $objSituacao->setStrDescricao('aguardando armazenamento banco RNA');
            $arrObjTStaTubo[] = $objSituacao;
    ...
    public function remover(InfosTubo $infosTubo){
        ...
        try{
            ...
            if ($infosTubo->getIdLocalFk() != null) {
                $objInfosTubo = new InfosTubo();
                $objInfosTubo->setIdLocalFk($infosTubo->getIdLocalFk());
                $arr_local = $objInfosTuboRN->listar($objInfosTubo);

                foreach ($arr_local as $info){
                    $info->setIdLocalFk(null);
                    $infosTubo = $objInfosTuboRN->alterar($info);
                }
            }*/

            $objExcecao->lancar_validacoes();
            $objInfosTuboBD = new InfosTuboBD();
            $objInfosTuboBD->remover($infosTubo,$objBanco);


            /*$objInfosTuboRN = new InfosTuboRN();
            if ($infosTubo->getIdLocalFk() != null) {
                foreach ($arr_local as $infolocal) {
                    $objLocalArmazenamentoTxt = new LocalArmazenamentoTexto();
                    $objLocalArmazenamentoTxtRN = new LocalArmazenamentoTextoRN();
                    $objLocalArmazenamentoTxt->setIdLocal($infolocal->getIdLocalFk());
                    $objLocalArmazenamentoTxtRN->remover($objLocalArmazenamentoTxt);
                }
            }*/
    ...
    public function consultar_aguardando_diagnostico(InfosTubo $infosTubo,$arrIdsPerfis){
        $objBanco = new Banco();
        try{
            $objExcecao = new Excecao();
            $objBanco->abrirConexao();
            ...
            $objExcecao->lancar_validacoes();
            $objInfosTuboBD = new InfosTuboBD();
            $arr = $objInfosTuboBD->consultar_aguardando_diagnostico($infosTubo,$arrIdsPerfis,$objBanco);
    ...
    public function procurar_sobra_volume($amostra){
        $objBanco = new Banco();
        try{
            $objExcecao = new Excecao();
            $objBanco->abrirConexao();
            ...
            $objExcecao->lancar_validacoes();
            $objInfosTuboBD = new InfosTuboBD();
            $arr = $objInfosTuboBD->procurar_sobra_volume($amostra,$objBanco);
