    ...
    */
    ...
    if (isset($_GET['idMontagem']) && isset($_GET['idMix'])) {
        ...
        */
        ...
        $objTipoLocalArmazenamento->setCaractereTipo(TipoLocalArmazenamentoRN::$TL_BANCO_AMOSTRAS_RNA);
        $arr = $objTipoLocalArmazenamentoRN->listar($objTipoLocalArmazenamento);
        $objTipoLocalArmazenamento = $arr[0];
            ...
            if (isset($_POST['btn_salvar_montagem'])) {
                ...
                /*echo "<pre>";
                  ...
                  echo "</pre>";*/
                    ...
                    $local->setIdTipoLocal($objTipoLocalArmazenamento->getIdTipoLocalArmazenamento());
                    ...
                    $infotubo->setObservacoes("Informando o local de armazenamento");
    ...
    if(isset($_POST['btn_salvar_montagem'])){
        ...
        if($objPlaca->getSituacaoPlaca() != PlacaRN::$STA_INVALIDA) {
            ...
            foreach ($objPlaca->getObjsAmostras() as $amostra){
                ...
                foreach ($objPlaca->getObjsPocosPlacas() as $pocoPlaca){
                    ...
                    if($amostra->getNickname() == $poco->getConteudo()){
                        ...
                        $objInfosTubo->setObservacoes("Posição: ".$poco->getLinha().$poco->getColuna());
            ...
            if ($objMontagemPlaca->getSituacaoMontagem() != MontagemPlacaRN::$STA_MONTAGEM_FINALIZADA) {
                $objMontagemPlaca->setDataHoraFim(date("Y-m-d H:i:s"));
                $arr_infos = array();
                $arr_alteracoes_aux = array();
                for ($i = 0; $i < count($arr_alteracoes); $i++) {
                    $arr_alteracoes_aux[] = $arr_alteracoes[$i];
                    $objInfosTubo = new InfosTubo();
                    $objInfosTubo->setIdInfosTubo(null);
                    ...
                    $objInfosTubo->setEtapa(InfosTuboRN::$TP_RTqPCR_MONTAGEM_PLACA);
                    $objInfosTubo->setSituacaoEtapa(InfosTuboRN::$TSP_FINALIZADO);
                    $objInfosTubo->setSituacaoTubo(InfosTuboRN::$TST_AGUARDANDO_BANCO_RNA);
                    $objInfosTubo->setDataHora(date("Y-m-d H:i:s"));
                    ...
                    $arr_alteracoes_aux[] = $arr_infos[0];
                    ...
                    $arr_alteracoes_aux[] = $arr_infos[1];*/
                ...
                $objMontagemPlaca->setObjMix($objMix);
                $objMontagemPlaca->setObjInfosTubo($arr_alteracoes_aux);
