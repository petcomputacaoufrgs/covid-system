    ...
    */
    ...
    if (isset($_GET['idPlaca']) && isset($_GET['idSolicitacao']) && isset($_GET['idMix'])) {
        ...
        if($error_qnt || count($tubos_inexistentes1) > 0 || count($tubos_inexistentes2) > 0 || count($tubos_inexistentes3) > 0) {
            ...
            $objPlacaAux->setSituacaoPlaca(PlacaRN::$STA_NO_MIX);
            $alert = Alert::alert_success("A placa tem situação <strong>REGULAR</strong>");
        }
    ...
    if(isset($_POST['btn_remover'])){
        header('Location: ' . Sessao::getInstance()->assinar_link('controlador.php?action=remover_amostras_da_placa&idMix='.$_GET['idMix'].'&idSolicitacao=' . $_GET['idSolicitacao'] . '&idPlaca=' . $objPlaca->getIdPlaca()));
        die();
    }
    ...
    if(isset($_POST['btn_salvar_mix'])){
        ...
        if($objPlaca->getSituacaoPlaca() != PlacaRN::$STA_INVALIDA ) {
            ...
            if ($objMix->getSituacaoMix() != MixRTqPCR_RN::$STA_TRANSPORTE_MONTAGEM) {
                ...
                $objMix->setSituacaoMix(MixRTqPCR_RN::$STA_AGUARDANDO_TABELAS_PROTOCOLOS);
                ...
                header('Location: ' . Sessao::getInstance()->assinar_link('controlador.php?action=mix_placa_RTqPCR&idSolicitacao=' . $arr_solicitacao[0]->getIdSolicitacaoMontarPlaca(). '&idPlaca=' . $objPlaca->getIdPlaca().'&idMix='.$objMix->getIdMixPlaca().'&idRelacionamento=1'));
                die();
            }else{
    ...
    //se já salvou o mix
    if((isset($_GET['idRelacionamento']) && $_GET['idRelacionamento'] == 1) || isset($_GET['idTabela'])) {
        ...
        if ($objProtocolo->getCaractere() == ProtocoloRN::$TP_NEWGENE) {
            ...
            $form .= '        </div>
                      ...
                      <div class="form-row">
                          <div class="col-md-12">
                                <button class="btn btn-primary" type="submit" '.$disabled.' style="width: 40%;margin-left: 30%;" name="btn_salvar_tabelas_operadores">SALVAR</button>
                          </div>
                      </div>  
                </form>';
        ...
        if(isset($_POST['btn_salvar_tabelas_operadores'])){
            ...
            for($j=0; $j <$divisoesPlaca; $j++) {
                for ($i = 0; $i < $tam; $i++) {
                    ...
                    $objRelMixOperador->setValor($_POST['txtOp' . $i . '_' . $arr_calculos[$j]->getObjOperador()[$i]->getIdOperador()]);
            ...
            $alert .= Alert::alert_success("Tabela do mix cadastrada");
            ...
            if ($objProtocolo->getCaractere() == ProtocoloRN::$TP_NEWGENE) {
                ...
                $form .= '        </div>
                      ...
                      <div class="form-row">
                          <div class="col-md-12">
                                <button class="btn btn-primary" type="submit" hidden style="width: 40%;margin-left: 30%;" name="btn_salvar_tabelas_operadores">SALVAR</button>
                          </div>
                      </div>  
                </form>';
...
echo    '  <form method="POST">
      ...
      if (isset($_GET['idPlaca']) && isset($_GET['idSolicitacao']) && isset($_GET['idMix'])) {
          ...
          if (Sessao::getInstance()->verificar_permissao('remover_amostras_da_placa') && !isset($_GET['idRelacionamento']) && !isset($_GET['idTabela'])) {
              echo '    
