...
try {
    ...
    */
    ...
    /*$dataPosterior = "2020-06-20 11:45:03";
    $dataAnterior = "2020-06-18 11:45:03";
    //echo "posterior: ".Utils::compararDataHora(date("Y-m-d H:i:s"), $dataPosterior)."\n";
    //echo "anterior: ".Utils::compararDataHora(date("Y-m-d H:i:s"), $dataAnterior)."\n";
    $arr = $objRTqPCR_RN->listar($objRTqPCR);
    foreach ($arr as $rtqpcr){
        echo "hrInicio: ".$rtqpcr->getDataHoraInicio()." -- ";
        echo Utils::getData($rtqpcr->getDataHoraInicio()) . ' ' . $rtqpcr->getHoraFinal()."\n";
        $data = Utils::getData($rtqpcr->getDataHoraInicio()) . ' ' . $rtqpcr->getHoraFinal();
        echo "= ".Utils::compararDataHora(date("Y-m-d H:i:s"), $data);
    }
    die();
    */
    ...
    */
    ...
    foreach ($arrRTqPCR as $RTqPCR) {
        ...
        if($segundos <= ($segundosEq/2)){
            ...
            $icon = '<button type="button" class="btn btn-secondary" style="border: none;" data-toggle="tooltip" data-placement="top" title="Falta mais da metade">
                        <i class="far fa-hourglass" style="color: green;"></i> 
                     </button>';
        }else if($segundos < 0){
            $icon = '<button type="button" class="btn btn-secondary" style="border: none;" data-toggle="tooltip" data-placement="top" title="Atraso no RTqPCR">
                        <i class="fas fa-hourglass" style="color: red;"></i>
                     </button>';
        }
        ...
        $html .= '   <td>'.Utils::getStrData($RTqPCR->getDataHoraInicio()).' - '.$RTqPCR->getHoraFinal().'</td>';
        ...
        if ($RTqPCR->getSituacaoRTqPCR() == RTqPCR_RN::$STA_EM_ANDAMENTO && Sessao::getInstance()->verificar_permissao('finalizar_RTqPCR'))  {
            $html .= '<td ><a href="' . Sessao::getInstance()->assinar_link('controlador.php?action=finalizar_RTqPCR&idRTqPCR=' . Pagina::formatar_html($RTqPCR->getIdRTqPCR()) . '&idPlaca=' . Pagina::formatar_html($objPlaca->getIdPlaca()). '&idEquipamento=' . Pagina::formatar_html($objEquipamento->getIdEquipamento())) .'">'.$icon.'</td>';
       }else if ($RTqPCR->getSituacaoRTqPCR() == RTqPCR_RN::$STA_ATRASADO && Sessao::getInstance()->verificar_permissao('finalizar_RTqPCR')) {
            $objResultado->setIdRTqPCRFk($RTqPCR->getIdRTqPCR());
            $arr_resultados = $objResultadoRN->listar($objResultado,1);
            if(count($arr_resultados) > 0){
                $html .= '<td ><a href="' . Sessao::getInstance()->assinar_link('controlador.php?action=finalizar_RTqPCR&idRTqPCR=' . Pagina::formatar_html($RTqPCR->getIdRTqPCR()) . '&idPlaca=' . Pagina::formatar_html($objPlaca->getIdPlaca()) . '&idEquipamento=' . Pagina::formatar_html($objEquipamento->getIdEquipamento()) . '&idComResultado=1') . '"><i class="fas fa-hourglass" style="color: red;"></i></td>';
            }else {
        ...
        if (Sessao::getInstance()->verificar_permissao('gerar_planilha_correta')) {
            $html .= '<td ><a href="' . Sessao::getInstance()->assinar_link('controlador.php?action=gerar_planilha_correta&idRTqPCR=' . Pagina::formatar_html($RTqPCR->getIdRTqPCR()) . '&idPlaca=' . Pagina::formatar_html($objPlaca->getIdPlaca()). '&idEquipamento=' . Pagina::formatar_html($objEquipamento->getIdEquipamento())) .'"><i class="far fa-file-excel" style="color: green;"></i></td>';

        }else{
            $html .= '<td ></td>';
        }
        if (Sessao::getInstance()->verificar_permissao('gerar_planilha_incorreta')) {
            $html .= '<td ><a href="' . Sessao::getInstance()->assinar_link('controlador.php?action=gerar_planilha_incorreta&idRTqPCR=' . Pagina::formatar_html($RTqPCR->getIdRTqPCR()) . '&idPlaca=' . Pagina::formatar_html($objPlaca->getIdPlaca()). '&idEquipamento=' . Pagina::formatar_html($objEquipamento->getIdEquipamento())) .'"><i class="far fa-file-excel" style="color: red;"></i></td>';
