...
try {
    ...
    */
    ...
    for($i=0; $i<($objAmostra->getTotalRegistros()/500); $i++){
        $color = '';
    ...
    foreach ($arrAmostras as $amostra){
        ...
        }else if($amostra->get_a_r_g() ==  AmostraRN::$STA_RECUSADA){
        ...
        $html .= '<tr>
                    ...
                    <td '.$color.'>' . Pagina::formatar_html(AmostraRN::mostrarDescricaoStaAmostra($amostra->get_a_r_g())) . '</td>';
        ...
        $html .= '   <td>' . Utils::getStrData($amostra->getDataColeta()) . '</td>';
