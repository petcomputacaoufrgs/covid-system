                ...
                if (count($perfil) > 1 && $paciente_sus == 's') {
                    ...
                    if (count($arr_grupo) == 0) {
                        ...
                        foreach ($arr_grupo as $grupo) { //todos os tubos originais
                            ...
                            if(isset($_POST['btn_confirmar'])) {
                                echo ' ';
                                //print_r($grupo->getAmostra());
                                $sumir_btn_confirmar = 's';
                                $sumir_btn_confirmar_novamente = 'n';
                                //echo 'idamostra:'.$grupo->getAmostra()->getIdAmostra();
                                //echo "#".$_POST['checkbox_' . $grupo->getAmostra()->getIdAmostra()]."#";
                                if ($_POST['checkbox_' . $grupo->getAmostra()->getIdAmostra()] == 'on') {
                                    echo ' ';
                                    $arr_amostras[] = $grupo->getAmostra();
                                    $arr_tubos[] = $grupo->getAmostra()->getObjTubo();
                            ...
                            //print_r($grupo);
                            if ($show) {
                                $data = explode("-", $grupo->getAmostra()->getDataColeta());
                                ...
                                $html .= '
                                        ...
                                        <div class="input-group col-md-12 " >
                                              ...
                                              <input type="text" disabled  class="form-control" value="Amostra ' . TuboRN::mostrarDescricaoTipoTubo($grupo->getAmostra()->getObjTubo()->getTipo()) . ' ' . $grupo->getAmostra()->getNickname() . ' - Data coleta: ' . $dia . '/' . $mes . '/' . $ano . '">
                                        </div>
                                        </div>';
                            }
                            ...
                            for($i=0; $i<count($arr_amostras); $i++){
                                ...
                                <div class="input-group col-md-12 " >
                                      <div class="input-group-prepend">
                                        ...
                                        <div class="input-group-text" >
                                          <input type="checkbox" checked  name="checkbox_' . $arr_amostras[$i]->getIdAmostra() . '"
                                          style="width: 50px;" aria-label="Checkbox for following text input">
                                        </div>
                                      </div>
                                      <input type="text" disabled  class="form-control" value="Amostra '.TuboRN::mostrarDescricaoTipoTubo($arr_amostras[$i]->getObjTubo()->getTipo()).' '. $arr_amostras[$i]->getNickname() . ' ---  Data coleta: ' . $arr_amostras[$i]->getDataColeta() .'">
                                </div>
                                </div>';*/
                            }
...
if ($button_selecionar == 'n') {
    ...
    if ($sumir_btn_confirmar == 'n') {
        ...
        echo '<div class="col-md-6"  style="margin-top: -10px;">
                    <button type="button" style="width:100%; margin-top: 20px;margin-left:0%;color:white;text-decoration: none;" class="btn btn-primary"  
