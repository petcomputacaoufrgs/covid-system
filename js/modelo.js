...
function validaModelo(){
    ...
    var strTipo = document.getElementById("idModelo");
    //var tipoAmostra = '<?=$objTipoAmostra->getTipo() ?>';
    
    var div_feedback = document.getElementById("feedback_modelo");
    ...
    if(strTipo.value.length == 0 || strTipo.value.length > 100 ){ // não digitou nada
        ...
        if(strTipo.value.length == 0 )div_feedback.innerHTML = " Digite um modelo. ";
        if(strTipo.value.length > 100 )div_feedback.innerHTML = " Digite um modelo com menos 100 caracteres. ";
    }
