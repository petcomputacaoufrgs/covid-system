...
function validaDoenca(){
     
    var strTipo = document.getElementById("idDoenca");
    //var tipoAmostra = '<?=$objTipoAmostra->getTipo() ?>';
    
    var div_feedback = document.getElementById("feedback_doenca");
    ...
    if(strTipo.value.length == 0 || strTipo.value.length > 100 ){ // não digitou nada
        ...
        if(strTipo.value.length == 0 )div_feedback.innerHTML = " Digite uma doença. ";
        if(strTipo.value.length > 100 )div_feedback.innerHTML = " Digite uma doença com menos 100 caracteres. ";
    }
